...
$ToolName = 'メディアダウンローダー'

# このフォルダがツール本体だと判断する目印。どれか 1 組が全部そろっていればよい。
# （exe 版だけを配布している）
$Editions = @(
    @{ Name = 'exe'; Markers = @('メディアダウンローダー.exe', '_internal') }
)
...
$OwnPaths = @(
    @{ Label = '設定ファイル'; Path = (Join-Path $env:LOCALAPPDATA 'johukku\media-downloader') }
)

# TEMP に残ることがあるもの。
# FFmpeg を取得するときの作業フォルダ（johukku-ffmpeg-*）は字幕作成ツールと同じ名前なので、
# どちらのものか区別できない。字幕作成ツールと同じく、ここでは消さない。
$OwnTempPatterns = @()
...
$SharedItems = @(
    @{
        ...
        Editions = @('exe')
        Note     = @('動画の取得と、mp3 / wav への変換に使います。',
                     'Whisper 字幕作成ツールなど、johukku 製ツールで共有しています。')
    }
)
